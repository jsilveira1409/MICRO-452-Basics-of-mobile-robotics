...
def computer_vision(img=None, object = None, show_image = False):
    ...
    return centers, contours, img_processed, img_hsv, img_smooth, mask
...
def cv_start(video_capture, exposure = None, show_image = False, nb_tries = 5):
    ...
    while(True):
        ...
        if ret == True:
            robot_center, robot_contour, _ , _, _, _ = computer_vision(frame, 'robot', show_image)
            _, obst_contours, _, _, _, _ = computer_vision(frame, 'obstacle', show_image)
            goal_center, goal_contours, _ , _, _, _= computer_vision(frame, 'goal', show_image)
